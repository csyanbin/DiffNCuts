...
from network.utils import trunc_normal_
...
class WCL(nn.Module):
    def __init__(self, dim_hidden=384, dim=384):
        super(WCL, self).__init__()
        self.head1 = ProjectionHead(dim_in=384, dim_out=dim, dim_hidden=dim_hidden)
    ...
    def forward(self, x1, x2, name, t=0.1):
        b = x1.size(0)
        backbone_feat1 = torch.mean(x1[:,1:,:], dim=1)
        feat1 = F.normalize(self.head1(backbone_feat1))

        mask1, label1 = self.build_connected_component(feat1@feat1.T)
        mask1 = mask1.float()
