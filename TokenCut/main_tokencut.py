...
from unsupervised_saliency_detection import dino
...
if __name__ == "__main__":
    ...
    model = dino.get_model(args.arch, args.patch_size).cuda()
    if args.eigen_train==1:        
    ...
    for im_id, inp in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            if "vit"  in args.arch:
                ...
                scales = [args.patch_size, args.patch_size]
                feat1_2d = model.forward(img.unsqueeze(0))
                nb_im, d, patch_nb, _ = feat1_2d.shape
                feats = feat1_2d.view(nb_im, d, -1).transpose(2,1)
                #feats = feat1_2d.view(nb_im, d, -1)
                ## Apply layernorm
                layernorm = torch.nn.LayerNorm(feats.size()[1:]).to(img.device)
