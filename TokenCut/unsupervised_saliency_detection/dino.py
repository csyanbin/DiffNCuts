...
def get_model(arch, patch_size, resnet_dilate=2):
    ...
    if "resnet" in arch:
        ...
        if 'small' in arch:
            model = moco_vit_small(num_classes=0)
        elif 'base' in arch:
        ...
        elif arch == "resnet50" or arch == "dino_resnet50":
            url = "dino/dino_resnet50_pretrain/dino_resnet50_pretrain.pth"
        if 'small' in arch:
            model = vit_small(num_classes=0, patch_size=patch_size)
        elif 'base' in arch:
            model = vit_base(num_classes=0)
...
class ViTFeat_Eigen(nn.Module):
    """ Vision Transformer """
    def __init__(self, pretrained_pth, feat_dim, vit_arch = 'base', vit_feat = 'k', patch_size=16, eigen_train=0):
        super().__init__()
        self.model = get_model(vit_arch, patch_size)
        self.feat_dim = feat_dim
        self.patch_size = patch_size
        self.vit_arch = vit_arch
        self.vit_feat = vit_feat

        print('eigen_train:', eigen_train)
        if eigen_train:  # Use eigen-trained checkpoint
            state_dict = torch.load(pretrained_pth, map_location="cpu")
            print("Use eigen trained ckpt:", pretrained_pth)

            if pretrained_pth.startswith("dino"): 
                weights_dict = state_dict
            else:
                weights_dict = {k.replace('module.', '').replace('net.', ''): v for k, v in state_dict['model'].items()}
            self.model.load_state_dict(weights_dict, strict=False)
        else:            # Use original checkpoint
    ...
    def forward(self, img) :
        ...
        with torch.no_grad():
            if 'resnet' not in self.vit_arch:
                feat_out = {}
                def hook_fn_forward_qkv(module, input, output):
                    feat_out["qkv"] = output
                self.model._modules["blocks"][-1]._modules["attn"]._modules["qkv"].register_forward_hook(hook_fn_forward_qkv)
                ...
                #features = k[:, 1:, :]
                features = k[:, 1:].transpose(1, 2).reshape(nb_im, self.feat_dim, -1)
                
            else:
                feat1_2d = self.model.forward(img)
                nb_im, d, patch_nb, _ = feat1_2d.shape
                #features = feat1_2d.view(nb_im, d, -1).transpose(2,1)
                features = feat1_2d.view(nb_im, d, -1)
                ## Apply layernorm
                layernorm = torch.nn.LayerNorm(features.size()[1:]).to(img.device)
